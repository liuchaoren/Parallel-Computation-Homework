...
uint FM[ORDER][ORDER];
// Compute Z-order
long fm(long xPos, long yPos)
{
  static const long MASKS[] = {0x55555555, 0x33333333, 0x0F0F0F0F, 0x00FF00FF};
  static const long SHIFTS[] = {1, 2, 4, 8};
...
double matrix_multiply(void) {
  ...
  //   // printf("%d\n",i);
  //   for (int k=0; k<P; k++){
  //     double temp = A[FM[i][k]];
  //     for (int j=0; j<M; j++){
  // 	C[FM[i][j]] += temp * B[FM[i][j]];
  //     }
  //   }
  // }
  ...
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < M; j++) {
      for (int k = 0; k < P; k++) {
	*(z_index((double*)C,i,j)) += (*(z_index((double*)A,i,k))) * (*(z_index((double*)B,k,j)));
      }
    }
  }
