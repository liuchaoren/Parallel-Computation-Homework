...
__attribute__((target(mic))) void filter(Pixel *myimg, Pixel *oimg, int rows, int cols)
{
...
double  apply_stencil(const int rows, const int cols, Pixel * in, Pixel * out) {

  double tstart, tend;

#pragma offload target(mic) in(in:length(rows*cols)) out(out:length(rows*cols))
  {
