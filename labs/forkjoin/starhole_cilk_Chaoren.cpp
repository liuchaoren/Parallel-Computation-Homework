...
int walker(long int seed, int x, int y, int stepsremaining) {
    ...
    int particles = 1;
    int particleList[stepsremaining];
    int stepstotal = stepsremaining;
    //initialize 
    ...
    for( ; stepsremaining>0 ; stepsremaining-- ) {
        ...
        if(doesSplit(&seedbuf, splitProb, x, y, radius)) {
            ...
            particleList[stepsremaining-1] = cilk_spawn walker(seed + newseed, x, y, stepsremaining-1);
        }
    ...
    for (int i=0; i<stepstotal; i++) 
        particles = particles + particleList[i];
    return particles;
}
